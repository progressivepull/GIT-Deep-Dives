...
user.name=John Cusey
user.email=john.j.cusey@irs.gov
...
remote.origin.url=git@github.enterprise.irs.gov:whwtb/Knowledge_Transfer_Session_GIT.git
...
Author: John Cusey <john.j.cusey@irs.gov>
...
3. ```Author: John Cusey <john.j.cusey@irs.gov>```
* ```Author```: This line specifies the author of the commit, the individual who made the changes in this commit.

* ```John Cusey <john.j.cusey@irs.gov>```: This provides the name and email address of the author. The author is the person who originally wrote the changes in a commit, which may differ from the committer in some scenarios.
